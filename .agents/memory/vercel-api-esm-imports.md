...
All relative ESM imports in the Vercel API dependency chain must use explicit `.js` extensions, including shared workspace-library indexes. Prefer named CommonJS-compatible exports when a package provides them. Let `express()` and `Router()` infer their types rather than annotating them as `Express` or `IRouter`.

**Why:** Local builds use bundler resolution, but Vercel typechecks the serverless entry with NodeNext. Extensionless imports and some default CommonJS imports pass locally but fail during deployment. Vercel can also resolve the exported `Express` and `IRouter` aliases differently from their factories' inferred types, causing valid `.use()`, `.get()`, and `.post()` calls to fail typechecking.
